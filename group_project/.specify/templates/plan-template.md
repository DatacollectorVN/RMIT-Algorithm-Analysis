...
**Storage**: [if applicable, e.g., files, JSON on disk, or N/A]  
...
- [ ] **Standard Library First**: No PyPI/third-party runtime or test dependencies;
      plan and file layout do not assume external packages.
- [ ] **PEP 8 & type hints**: Approach is compatible with strict typing and PEP 8;
      public API surfaces identified for docstrings and hints.
- [ ] **Functional-first modularity**: Boundaries (modules/packages) are clear;
      shared mutable state is minimal and justified.
- [ ] **Complexity & memory**: Primary data paths target O(n)-time (or better)
      typical behavior; extra space proportional unless justified in Complexity
      Tracking below.
- [ ] **Documentation**: Plan names modules/functions that will use Google-style
      docstrings.
- [ ] **Errors**: Custom exception hierarchy covers expected domain/contract
      failures for this feature.
- [ ] **Testing**: Test strategy uses `unittest` (stdlib) only.
